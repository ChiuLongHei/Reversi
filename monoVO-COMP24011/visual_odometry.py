...
def featureMatching(image_ref, image_cur, matching_algorithm, threshold_value, output_path):
    ...
    imgKp1 = cv2.drawKeypoints(image_ref,kp1,None)
    imgKp2 = cv2.drawKeypoints(image_cur,kp2,None)

    bf=cv2.BFMatcher()
    matches=bf.knnMatch(des1,des2,k=2)
    #img4 = cv2.drawMatchesKnn(image_ref, kp1, image_cur, kp2, matches[:50], None, flags=2)
    ...
    if matching_algorithm==2:
    	for m,n in matches:
    		...
    		if m.distance<n.distance:
    			good.append([m])
    			result=result+str(round(m.distance, 2))+" \n"
    		count=count+1
    ...
    img3 = cv2.drawMatchesKnn(image_ref, kp1, image_cur, kp2, good, None, flags=2)
    fig = plt.figure()
